...
export function SiteFooter() {
  return <footer className="bg-ink text-primary-foreground">
    <div className="grid min-h-[68dvh] lg:grid-cols-2">
      ...
      <div className="flex flex-col justify-between px-6 py-14 md:px-12 md:py-20 lg:px-16"><div><p className="text-[.68rem] font-semibold uppercase tracking-[.22em] text-accent">Your bridal fitting</p><h2 className="mt-6 max-w-xl font-display text-5xl leading-[.92] md:text-7xl">Bring the dress. <em className="text-accent">We’ll perfect the fit.</em></h2><p className="mt-7 max-w-lg text-base leading-8 text-primary-foreground/65">Tell us about your wedding dress, bridesmaid dress or alteration. We will advise on the work required and arrange an unhurried fitting.</p><Button asChild variant="ivory" size="lg" className="mt-9"><Link to="/contact">Book a fitting <ArrowUpRight/></Link></Button></div><div className="mt-16 flex items-start gap-3 border-t border-primary-foreground/15 pt-6 text-sm text-primary-foreground/55"><MapPin className="mt-0.5 size-4 text-accent"/><p>{businessDetails.location}<br/><span className="text-xs">Full atelier address to be confirmed</span></p></div></div>
    </div>
